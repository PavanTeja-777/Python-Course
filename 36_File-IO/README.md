...
1. **text (t)**: 
t mode is used to handle text files. t refers to the text mode. There is no difference between r and rt or w and wt since text mode is the default. The default mode is 'r' (open for reading text, synonym of 'rt' ).
2. **binary (b)**: used to handle binary files (images, pdfs, etc).
